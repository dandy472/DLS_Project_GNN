...
По согласованию с куратором проекта, дальнейшая работа по GML будет построена на исследовании социальных сетей с использованием GNN.   
...
Симметрии (в данном случае не заложенные, а индуцированные):
...
•	Персистентные диаграммы для размерностей 0 и 1 (H₀ — компоненты связности, H₁ — циклы)
...
### Выводы: ###
1.	Пользовательские круги не являются структурными сообществами -силуэтный коэффициент близок к нулю или отрицателен
2.	Семантическая связность важнее структурной — время жизни циклов в пространстве признаков выше, чем в структурном пространстве
...
•	Компоненты связности (H₀): 1476 компоненты из 1500 узлов (98%) — граф становится связным при малых порогах расстояния Жаккара
•	Циклические структуры (H₁): 712 циклов (47.5% от выборки) с характеристиками:
...
1.	Precision-Recall tradeoff: Модель стала и точнее и полнее
2.	Неоднородность улучшения по сетям:
o	Ego 1912: F1 улучшился с 0.0254 до 0.0297 (+17%)
o	Ego 3980: F1 улучшился с 0.0311 до 0.0377 (+21,2%)
3.	Значения F1 всё ещё низкие

4. Выводы
1. Топологические признаки улучшают предсказательную способность GNN
2. Персистентные диаграммы и топологические инварианты действительно кодируют информацию о структуре сообществ
3. Глобальные признаки (pagerank, betweenness) оказались менее полезными, чем локальные структурные признаки
